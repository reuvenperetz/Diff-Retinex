...
class MMSEDataset(Dataset):
    def __init__(self, dataroot, split="train", with_r=True, with_l=True,
                 require_high=True, with_r_high=False, with_l_high=False):
        self.split = split
        self.with_r = with_r
        self.with_l = with_l
        ...
        self.r_high_paths = _get_paths(os.path.join(base, "R_high")) if require_high and with_r_high else None
        self.l_high_paths = _get_paths(os.path.join(base, "L_high")) if require_high and with_l_high else None
        ...
        if require_high:
            ...
            if with_r_high and len(self.low_paths) != len(self.r_high_paths):
                raise RuntimeError("low and R_high counts do not match")
            if with_l_high and len(self.low_paths) != len(self.l_high_paths):
    ...
    def __getitem__(self, idx):
        ...
        if self.high_paths:
            ...
            sample["high"] = self.to_tensor(high)
